...
### 3.4 Tree::tickRoot 完整执行链路

```mermaid
flowchart TD
    A["tree.tickRootWhileRunning()"] --> B{"status == RUNNING?"}
    B -->|YES| C["tree.tickRoot()"]
    B -->|NO| F["结束"]
    
    C --> C1["initialize（首次）"]
    C1 --> C1a["创建 WakeUpSignal"]
    C1 --> C1b["为每个节点设置 WakeUpSignal"]
    
    C1b --> C2["rootNode() -> executeTick()"]
    C2 --> C2a["执行 pre_condition_callback_"]
    C2a --> C2b["执行 tick() （子类具体实现）"]
    
    %% 节点类型分流
    C2b --> D1["ControlNode"]
    C2b --> D2["DecoratorNode"]
    C2b --> D3["StatefulActionNode"]
    C2b --> D4["SyncActionNode"]
    C2b --> D5["AsyncActionNode"]
    C2b --> D6["CoroActionNode"]
    
    %% 节点内部逻辑
    D1 --> D1a["循环 tick 子节点: child -> executeTick()"]
    D2 --> D2a["tick 子节点 -> 自动 resetStatus()"]
    D3 --> D3a["IDLE → onStart() / RUNNING → onRunning()"]
    D4 --> D4a["直接返回结果（禁止返回 RUNNING）"]
    D5 --> D5a["IDLE → 启动工作线程执行"]
    D6 --> D6a["创建或恢复协程执行"]
    ...
    C2c["执行 post_condition_callback_"] --> C2d["setStatus(new_status)"]
    
    %% 状态变更通知
    C2d --> C2e["state_mutex_ 保护写入"]
    C2e --> C2f["condition_variable.notify_all()"]
    C2f --> C2g["state_change_signal_.notify(...)"]
    C2g --> C2h["触发所有 Logger 的 callback"]
    
    %% 休眠与唤醒机制
    C2h --> E["tree.sleep(10ms)"]
    E --> E1["wake_up_ -> waitFor(10ms)"]
    
    E1 -->|被 emitStateChanged 唤醒| E2["立即返回"]
    E1 -->|超时| E3["超时返回"]
